...
class Path(PurePath):
    ...
    def print_big_dir_and_big_file(self, top_n=5):
        """Print top_n big directory and top_n big file of each directory.
        """
        size_table1 = sorted(
            [(p, p.dirsize) for p in self.select_dir(recursive=False)],
            key=lambda x: x[1],
    ...
    def mirror_to(self, dst):
        """Create a copy of directory and it's subfolder and files. They share
        exactly the same structure and name. But all file are empty file.
        """
        src = self.abspath
